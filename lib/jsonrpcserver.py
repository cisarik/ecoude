...
class JSONSerializer:
    ...
    def deserialize(self, string):
        ...
        if (parameters==None):
            parameters=[]
        json_obj={"method": str(params.get('method')[0]), "params": parameters , "id": 1}
        return json.JSONDecoder().decode(AugmentedJSONEncoder().encode(json_obj))
    ...
    def process_request(self, request):
        ...
        for pattern in self.jsonrpc_urlpatterns:
            ...
            if match:
                ...
                result_str = self._dispatch_rpc_call(pattern._callback_str, request.raw_post_data, args, kwargs, request)
                return HttpResponse(result_str)

    def _dispatch_rpc_call(self, module_name, raw_post_data, args, kwargs, request):
        response_dict = {}
        try:
            ...
            if call_id:
                ...
                return ""
                    #call_data["params"]+=request
            func_name = str(call_data["method"])
                    
            func_params = [request]
                    
            func_params.extend(list(call_data["params"]))
                    #func_params+=request
                    #func_params+=call_data["params"]  
                    #func_params = list(call_data["params"])
                    #func_params = list(call_data["params"])
                    #req=list(request)
                    #func_params.append(request)

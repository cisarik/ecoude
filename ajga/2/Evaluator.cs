    ...
    public class Evaluator : IEvaluator<int, int>
    {
        ...
        public int Evaluate(IGenome<int, int> genome)
        {
            ...
            foreach (int p in genome.Genes)
            {
                ...
                {
                    value += System.Convert.ToInt32(Math.Sqrt((this.positions[p].X - position.X) * (this.positions[p].X - position.X) + (this.positions[p].Y - position.Y) * (this.positions[p].Y - position.Y)));
                    position = this.positions[p];
                }
            }
